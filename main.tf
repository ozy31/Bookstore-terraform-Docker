...
provider "github" {
    token = XXXXXXXXXXXXX #Must add here !!!!!!
...
resource "aws_instance" "tf-docker-ec2" {
    ...
    key_name = "Firstkey"
    security_groups = ["XXXXXXXXXX"] # Name Security Group !!!!!!!!!!!!
    tags = {
        Name = "ozy-Web Server of Bookstore" #Optional 
    }
    user_data = <<-EOF
          #! /bin/bash
          yum update -y
          ...
          FOLDER="https://$TOKEN@raw.githubusercontent.com/XXXXXX/bookstore-repo/main/"
...
resource "aws_security_group" "tf-docker-ec2-sec-gr" {
    name = "XXXXXXXXXXXXX" #define sec group here !!!!!
    tags = {
